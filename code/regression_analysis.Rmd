...
## setup whether to use log-transformation for two independent variables: num_terms, monthly_tweets
- finally, we chose the original or raw scale of "num_terms" and "monthly_tweets" since its AIC is smallest
```{r}
use_log_form_of_num_terms = F
#use_log_form_of_num_terms = T

use_log_form_of_monthly_tweets = F
#use_log_form_of_monthly_tweets = T

remove_those_tweeting_too_rarely_or_too_frequently = F 
#remove_those_tweeting_too_rarely_or_too_frequently = T # no difference from FALSE
monthly_tweets_TH_bottom = 5
monthly_tweets_TH_top = 400
```
...
create_filename_for_output = function(namebase, ext='.pdf') {
  ...
  tweets_str = paste0('__tweets', ifelse(use_log_form_of_monthly_tweets, 'LOG', 'RAW'))
  allmembers_str = paste0('__allmembers', ifelse(remove_those_tweeting_too_rarely_or_too_frequently, 'NO', 'YES'))
  filename = paste(c(namebase, terms_str, tweets_str, allmembers_str, '.', ext), collapse = '')
...
run_regression_model = function(data, print_summary=T, show_tictoc=T, fixed_factors=NULL) {
  if (is.null(fixed_factors)) {
    ...
    fixed_part_monthly_tweets = ifelse(use_log_form_of_monthly_tweets, "monthly_tweets_log", "monthly_tweets")
    
    fixed_parts2 = paste(c(fixed_part_num_terms, fixed_part_monthly_tweets, "followers_log"), collapse = '+')
...
- age: (today - birthday) / 365.25, which depends on when the data is gendered 
- num_terms: how many terms served in Congress
- followers_log: logarithm of followers (log-transform: normalize highly skewed distribution)

```{r}
# the following data is generated in "helpers.py" > merge_member_and_prediction_data()

df = read.csv('../data/final_data_for_regression_analysis.csv')
print(nrow(df))
...
df[, monthly_tweets:=.N, by = list(bio_id, ym)]

# so far it seems okay to keep all of them
if (remove_those_tweeting_too_rarely_or_too_frequently) {

  dfu = copy(unique(df, by=c('bio_id', 'ym')))
  dfu[, avg_monthly_tweets:=mean(monthly_tweets), by='bio_id']
  dfu_ = unique(dfu, by=c('bio_id'))
  bids_too_few = dfu_[avg_monthly_tweets<monthly_tweets_TH_bottom]$bio_id
  bids_too_much = dfu_[avg_monthly_tweets>monthly_tweets_TH_top]$bio_id

  print(paste0('- removed those whose avg monthly tweets < ', monthly_tweets_TH_bottom, ' : ', length(bids_too_few)) ) 
  print(paste0('- removed those whose avg monthly tweets > ', monthly_tweets_TH_top, ' : ', length(bids_too_much)) ) 
  
  df = df[!(bio_id %in% bids_too_few)]
  df = df[!(bio_id %in% bids_too_much)]
  print(paste0('- remained: ', nrow(dfu_) - length(bids_too_much) - length(bids_too_few)) )
  print(nrow(df))
}


df$monthly_tweets_log = log1p(df$monthly_tweets)
...
For continuous predictors (age, num_terms_log, followers_log, monthly_tweets_log), they are held constant at their mean value (e.g., age=60).
...
- monthly_tweets / monthly_tweets_log
- followers_log
```{r}

dfu_ = copy(unique(df, by=c('bio_id', 'ym')))
dfu_[, avg_monthly_tweets:=median(monthly_tweets), by='bio_id']
dfu = unique(dfu_, by=c('bio_id'))
dfu$avg_monthly_tweets_log = log1p(dfu$avg_monthly_tweets)
dfu$avg_monthly_tweets_sqrt = sqrt(dfu$avg_monthly_tweets)


cbPalette <- c("#56B4E9", "#E69F00", "#F55E00",  "#0072B2",  "#CC79A7", "#BBBBBB", '#666666')

#mylist <- list('age'=5, 'followers_log'=.5, 'num_terms'=1,  'num_terms_log'=.5, 'avg_monthly_tweets'=20, 'avg_monthly_tweets_log'=.15, 'avg_monthly_tweets_sqrt'=.5)

mylist <- list('age'=5, 'followers_log'=.5, 'num_terms'=1, 'avg_monthly_tweets'=20)
#mylist <- list('avg_monthly_tweets'=20)
...
for (i in seq_along(mylist)) {
  ...
  fpath_output = paste(c(folder_of_results, 'distri_', var, allmembers_str, '.pdf'), collapse = '')
...
```{r}
data = df[ym=='2020-06',]
factors = "gender + party + chamber + age + num_terms + monthly_tweets + followers_log"
...
data_numeric = data[, .(age, num_terms, monthly_tweets, followers_log)]
